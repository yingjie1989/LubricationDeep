...
InputParameters validParams<ReactionSUPG>()
{
  ...
  params.addCoupledVar("coupled", 0, "Coupled variable");
  ...
  params.addParam<Real>("timeStep",1.0,"addtional viscous term");
  params.addParam<MaterialPropertyName>("mesh_size","hele","meshSize");
  params.addParam<MaterialPropertyName>("D_param",0,"");
  return params;
}

ReactionSUPG::ReactionSUPG(const InputParameters & parameters) :
    Kernel(parameters),
...
Real
ReactionSUPG::computeQpResidual()
{
    ...
    if(_dim == 2)
   	...
   	return _coeff * _tau* ( _grad_test[_i][_qp](0) * (_coupled_x[_qp]/_uN) + _grad_test[_i][_qp](1) * ( _coupled_y[_qp]/_uN)  ) * ( _u[_qp] - _coupled[_qp] )/_timeStep;

    return _coeff * _tau*( _grad_test[_i][_qp](0)*signu )*( _u[_qp] - _coupled[_qp] )/_timeStep;
} 

Real
ReactionSUPG::computeQpJacobian()
{
    ...
    if(_dim == 2)
        ...
        return _coeff * _tau* ( _grad_test[_i][_qp](0) * (_coupled_x[_qp]/_uN) + _grad_test[_i][_qp](1) * ( _coupled_y[_qp]/_uN)  ) * _phi[_j][_qp]/_timeStep;


    return _coeff * _tau*( _grad_test[_i][_qp](0)*signu )*_phi[_j][_qp]/_timeStep;

}


Real
ReactionSUPG::computeQpOffDiagJacobian(unsigned int jvar)
{
	return 0.0;
}
