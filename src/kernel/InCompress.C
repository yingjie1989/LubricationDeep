...
InCompress::InCompress(const InputParameters & parameters) :
   Kernel(parameters),
...
Real
InCompress::computeQpResidual()
{
      return _test[_i][_qp] * (_coupled_gradu[_qp](0) + _coupled_gradv[_qp](1) + _grad_u[_qp](_dim) );

}

Real
InCompress::computeQpJacobian()
{
...
Real
InCompress::computeQpOffDiagJacobian(unsigned int jvar)
{
